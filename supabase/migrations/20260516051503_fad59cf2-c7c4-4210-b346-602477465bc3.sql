...
create policy "Users can insert own role"
  on public.user_roles for insert with check (auth.uid() = user_id and role <> 'admin');
create policy "Admins manage all roles"
  on public.user_roles for all using (public.has_role(auth.uid(),'admin'));

-- Guide listings (linked to a user)
...
create policy "Approved guides are public"
  on public.guides for select using (status = 'approved' or auth.uid() = user_id or public.has_role(auth.uid(),'admin'));
create policy "Users can create own guide listing"
  on public.guides for insert with check (auth.uid() = user_id);
create policy "Users can update own guide listing"
  on public.guides for update using (auth.uid() = user_id);
create policy "Admins manage guides"
  on public.guides for all using (public.has_role(auth.uid(),'admin'));
...
-- Auto create profile + default tourist role on signup
